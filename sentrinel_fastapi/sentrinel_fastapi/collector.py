...
class Collector:
    ...
    def _ensure_started(self) -> None:
        ...
        Gunicorn and uWSGI fork workers after loading the application, and so
        does gunicorn running uvicorn workers. A thread
...
#: Where a collector created without a config gets one. Each framework package
#: installs its own reader — Django's settings — and without one it is the
#: environment alone.
_config_loader: Callable[[], Config] | None = None


def set_config_loader(loader: Callable[[], Config] | None) -> None:
    """Install how this framework reads its settings when nobody passed a config."""
...
def get_collector(config: Config | None = None) -> Collector:
    ...
    from places that can each run more than once — every middleware instance a
    framework builds, and every log record emitted outside a request. A second
    caller must join the existing collector, not evict the first.

    The one exception: a collector that was created unconfigured adopts the
    first configured one it is offered. A log line written at startup — before
    an ASGI app builds its middleware, which it does on the first request —
    creates the collector from the environment alone. Keeping that forever
    would ignore the settings the middleware was given, and send nothing.
    """
    global _collector
    if _collector is None:
        with _collector_lock:
            if _collector is None:
                _collector = Collector(config or (_config_loader or load)())
                atexit.register(_collector.shutdown)
                return _collector
    if config is not None and config.configured and not _collector.config.configured:
        with _collector_lock:
            if not _collector.config.configured:
                _collector.config = config
    return _collector
