...
emits and ships them with the request they were written during — same request id, same
...
one — a management command, a worker, startup — go straight to the collector.
"""
...
class SentrinelLogHandler(logging.Handler):
    ...
    def _emit(self, record: logging.LogRecord) -> None:
        ...
        if state is not None:
            ...
            line["seq"] = state["seq"]
            state["seq"] += 1
            # Held, not sent: the middleware stamps the consumer that the handler
