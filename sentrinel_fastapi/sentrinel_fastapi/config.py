...
``SENTRINEL`` block from settings, FastAPI takes keyword arguments to the
middleware — and hands it to ``load()``::
...
def _compile(patterns: Iterable[Any]) -> list[Pattern[str]]:
    """Accept strings or pre-compiled patterns, as a settings file may hold either."""
...
#: Every key ``load()`` reads. A framework that takes options as keyword
#: arguments checks against this, so a typo is an error at startup rather than
#: a setting that is silently ignored and a dashboard that stays empty.
SETTING_KEYS: frozenset[str] = frozenset(
    """SERVER_URL APP_NAME MODULE TUNNEL_MODULE ENV API_KEY VERSION ENABLED DEBUG
    FLUSH_INTERVAL MAX_BUFFER TIMEOUT CAPTURE_REQUESTS CAPTURE_ERRORS CAPTURE_LOGS
    SAMPLE_RATE SLOW_REQUEST_MS LOG_REQUEST_HEADERS LOG_REQUEST_BODY LOG_RESPONSE_BODY
    MAX_BODY_BYTES EXCLUDE_PATHS MASK_HEADERS MASK_FIELDS CONSUMER_IDENTIFIER""".split()
)
